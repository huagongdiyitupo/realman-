...
class Realman(BaseAgent):
    ...
    keyframes = dict(
        rest=Keyframe(
            qpos=np.array([
                1.83956963, 1.17592755, 1.5852669, -1.52353741, -0.37200622, -0.36985699, -0.14799367,  # left arm
                1.2246057, -1.23747981, -1.36873581, 1.53278871, -0.14879535, 0.42349776, -2.70365,      # right arm
            ]),
            pose=sapien.Pose(),
        )
    )
...
class RealmanGraspCubeDomainRandomizationConfig:
    ...
    the default, it will set the robot parts to white and motors to black."""
...
class RealmanGraspCubeEnv(BaseDigitalTwinEnv):
    """
    **Task Description:**
    A simple task where the objective is to grasp a cube with the Realman dual-arm robot and bring it up to a target rest pose.
    ...
    **Success Conditions:**
    ...
    SUPPORTED_ROBOTS = ["Realman"]  # 修改：支持的机器人类型
    SUPPORTED_OBS_MODES = ["none", "state", "state_dict", "rgb+segmentation"]
    agent: Realman  # 修改：类型注解

    def __init__(
        self,
        *args,
        robot_uids="Realman",  # 修改：机器人ID
        control_mode="pd_joint_pos",  # 修改：控制模式（根据你的机器人支持的控制模式）
    ...
    def _load_agent(self, options: dict):
        ...
        super()._load_agent(
            options,
            sapien.Pose(p=[0, 0, 0], q=euler2quat(0, 0, 0)),  # 修改：根据你的机器人调整初始姿态
            build_separate=True
            if self.domain_randomization
    ...
    def _load_scene(self, options: dict):
        ...
        for i in range(self.num_envs):
            ...
            builder.add_box_collision(
                half_size=[half_sizes[i]] * 3, material=material, density=200
            )
        ...
        self.table_pose = Pose.create_from_pq(
            p=[-0.12 + 0.737, 0, -0.9196429], q=euler2quat(0, 0, 0)  # 可能需要调整
        )
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # 修改：使用你的机器人的初始关节配置，添加噪声
            noise_scale = self.domain_randomization_config.initial_qpos_noise_scale
            self.agent.robot.set_qpos(
                self.rest_qpos + torch.randn(size=(b, self.rest_qpos.shape[-1])) * noise_scale
            )
            self.agent.robot.set_pose(
                Pose.create_from_pq(p=[0, 0, 0], q=euler2quat(0, 0, 0))  # 根据需要调整
            )
    ...
    def _get_obs_extra(self, info: Dict):
        # 修改：根据你的机器人调整观测
        obs = dict(
            dist_to_rest_qpos=self.agent.controller._target_qpos
            - self.rest_qpos,  # 可能需要调整维度
        )
        if self.obs_mode_struct.state:
            obs.update(
                is_grasped=info["is_grasped"],
                obj_pose=self.cube.pose.raw_pose,
                tcp_pos=self.agent.tcp_pos,  # 注意：确保你的机器人有tcp_pos属性
    ...
    def evaluate(self):
        # 修改：评估函数需要根据你的机器人调整
        # 注意：你需要确保机器人有tcp_pos和is_grasping方法
        ...
        distance_to_rest_qpos = torch.linalg.norm(
            target_qpos - self.rest_qpos, axis=-1  # 可能需要调整维度
        )
        ...
        # 修改：检测机器人是否接触桌子，需要根据你的机器人link名称调整
        # 你需要替换为你的机器人的手指/夹爪link名称
        l_contact_forces = self.scene.get_pairwise_contact_forces(
            self.agent.robot.links_map["panda_finger_joint1"],  # 替换为你的左手指link
            self.table_scene.table
        )
        r_contact_forces = self.scene.get_pairwise_contact_forces(
            self.agent.robot.links_map["panda_finger_joint2"],  # 替换为你的右手指link
            self.table_scene.table
        )
        lforce = torch.linalg.norm(l_contact_forces, axis=1)
        rforce = torch.linalg.norm(r_contact_forces, axis=1)
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        tcp_to_obj_dist = torch.linalg.norm(
            self.cube.pose.p - self.agent.tcp_pose.p, axis=1  # 注意：确保有tcp_pose属性
        )
